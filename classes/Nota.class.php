...
class Nota extends Model
{
    ...
    public function validate($data = array())
    {
        ...
        //Image Upload
        if ($data["form"] && isset($_FILES["imagen"]) && $_FILES["imagen"]["size"] > 0) {
            try {
                $config = Registry::getConfig();
        ...
        //Archivo Upload
        if ($data["form"] && isset($_FILES["archivo"]) && $_FILES["archivo"]["size"] > 0) {
            try {
                //Eliminamos el anterior
                $this->deleteArchivo();
                //Subimos la nueva
                $this->archivo = substr(md5(uniqid()),0,6)."_".$_FILES["archivo"]["name"];
                move_uploaded_file($_FILES["archivo"]["tmp_name"], $this->getArchivoPath());
            } catch (ImageUploaderException $e) {
